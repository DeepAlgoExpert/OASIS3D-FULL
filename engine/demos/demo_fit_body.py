...
def main(args):
    ...
    for i, batch in enumerate(tqdm(testdata, dynamic_ncols=True)):
        ...
        if args.saveObj:
            ...
            obj_path = os.path.join(savefolder, name, f'{name}.obj')
            visualizer.save_obj(obj_path, opdict)
            # Rotate the saved .obj file
            # Rotate the saved .obj file
            mesh = trimesh.load(obj_path)
            mesh.vertices = np.dot(mesh.vertices, [[1, 0, 0], [0, -1, 0], [0, 0, -1]])  # Rotate vertices
        ...
        if args.tPose:
            if 'Tpose_vertices' not in opdict.keys():
                 opdict['Tpose_vertices'] = pixie.decode_Tpose(codedict)
            visualizer.save_obj(os.path.join(savefolder,  f'{name}.obj'), opdict,Tpose=True)
        if args.saveParam:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lightTex', default=False, type=lambda x: x.lower() in ['true', '1'],
    ...
    parser.add_argument('--tPose', default=True, type=lambda x: x.lower() in ['true', '1'],
                        help='whether to save outputs as tPose obj, \
                            Note that saving objs could be slow' )
